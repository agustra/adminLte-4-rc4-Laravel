...
const generalConfig = {
    // Table & URLs
    tableId: "#table-controller-permissions",
    urlWeb: "/admin/controller-permissions/",
    urlApi: "/api/controller-permissions",
    deleteMultipleUrl: "/api/controller-permissions/multiple/delete",

    // Table Configuration
    tableName: "controller-permissions",
    exportColumns: ["controller", "method"],
    createButtonText: "Add Mapping",
    itemName: "controller-permissions", // untuk bulk delete
    ...
    // Console Messages
    apiErrorPrefix: "Controller Permissions API Error:",
...
function createTableConfig() {
    ...
    return {
        ...
        columns: [
            ...
            { data: "controller", title: "Controller", orderable: true },
            { data: "method", title: "Method", orderable: true },
            {
                data: "permissions",
                title: "Permissions",
                orderable: true,
                render: (data) => {
                    ...
                    try {
                        ...
                        // Jika gagal parse, tampilkan data asli
                        return data;
                    }
                },
            },
...
function initializeControllerPermissionsTable() {
    ...
    document.body.addEventListener("click", (e) => {
        ...
        if (createBtn) {
            ...
            if (form) {
                ...
                // Check for Laravel method spoofing
                const methodField = formData.get('_method');
                if (methodField && typeof methodField === "string") {
                    method = methodField.toUpperCase();
                    formData.delete('_method'); // Remove _method from data
                }
...
document.addEventListener("DOMContentLoaded", () => {
    const tableInstance = initializeControllerPermissionsTable();
...
document.addEventListener("shown.bs.modal", async (e) => {
    ...
    // TomSelect untuk permissions
    try {
        const { initializePermissionsTomSelect } = await import(
            "@tomselect/permissionsSelect.js"
        );
        setTimeout(() => {
            const permissionsElement = modal.querySelector("#permissions");
            if (permissionsElement) {
                // Ambil selected values dari HTML select
                const selectedValues = Array.from(
                    permissionsElement.selectedOptions
                ).map((opt) => opt.value);

                const tomSelectInstance = initializePermissionsTomSelect(
                    permissionsElement,
                    {
                        multiple: true,
                        createSingle: false,
                ...
                // Set selected values setelah TomSelect ready
                if (selectedValues.length > 0 && tomSelectInstance?.ts) {
                    setTimeout(() => {
                        selectedValues.forEach((value) => {
                            tomSelectInstance.ts.addOption({ name: value });
                            tomSelectInstance.ts.addItem(value, true);
                        });
                    }, 200);
                }
            }
        ...
        console.error("Error loading permissions select:", error);
    }
});

// Export default function untuk dynamic loading
export default function initControllerPermissionsModule() {
    console.log("🔐 Controller Permissions module initialized successfully!");
